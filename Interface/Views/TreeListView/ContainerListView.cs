...
namespace DeOps.Interface.TLV
{
	...
	public class ContainerListView: UserControl
	{
		#region Events
		...
		protected virtual void DrawBorder(Graphics g, Rectangle r)
		{
			...
			{
				g.DrawRectangle(SystemPens.WindowFrame, r.Left, r.Top, r.Width, r.Height);
			}
			else if (borderstyle == BorderStyle.Fixed3D)
			{
				ControlPaint.DrawBorder3D(g, r.Left, r.Top, r.Width, r.Height, Border3DStyle.Flat);
			}
		...
		protected virtual void DrawHeaders(Graphics g, Rectangle r)
		{
			...
			if (headerStyle != ColumnHeaderStyle.None)
			{
                g.FillRectangle(new SolidBrush(SystemColors.Control), r.Left + borderWid, r.Top + borderWid, r.Width - borderWid * 2, headerBuffer);
                ...
                g.Clip = new Region(new Rectangle(r.Left + borderWid, r.Top + borderWid, r.Width - borderWid*2, r.Top + headerBuffer));
				for (i=0; i<columns.Count; i++)
				{
                    if ((lp_scr + last + columns[i].Width > r.Left + borderWid)
                        && (lp_scr + last < r.Left + r.Width - borderWid))
					{						
						if (headerStyle == ColumnHeaderStyle.Clickable && columns[i].Pressed)
                            System.Windows.Forms.ControlPaint.DrawButton(g, lp_scr + last, r.Top + borderWid, columns[i].Width, r.Top + headerBuffer, ButtonState.Flat);
						else
                            System.Windows.Forms.ControlPaint.DrawButton(g, lp_scr + last, r.Top + borderWid, columns[i].Width, r.Top + headerBuffer, ButtonState.Normal);
					
						if (columns[i].Image != null)
						{
                            g.DrawImage(columns[i].Image, new Rectangle(lp_scr + last + 4, r.Top + borderWid + 1, 16, 16));
                            g.DrawString(TruncatedString(columns[i].Text, columns[i].Width, 25, g), this.Font, SystemBrushes.ControlText, (float)(lp_scr + last + 22), (float)(r.Top + borderWid + 3));
						}
						else
						{
							string sp = "";
							if (columns[i].TextAlign == HorizontalAlignment.Left)
                                g.DrawString(TruncatedString(columns[i].Text, columns[i].Width, 0, g), this.Font, SystemBrushes.ControlText, (float)(lp_scr + last + 4), (float)(r.Top + borderWid + 3));
							else if (columns[i].TextAlign == HorizontalAlignment.Right)
							{
								sp = TruncatedString(columns[i].Text, columns[i].Width, 0, g);
                                g.DrawString(sp, this.Font, SystemBrushes.ControlText, (float)(lp_scr + last + columns[i].Width - Utilities.MeasureDisplayStringWidth(g, sp, this.Font) - 4), (float)(r.Top + borderWid+3));
							}
							else
							{
								sp = TruncatedString(columns[i].Text, columns[i].Width, 0, g);
                                g.DrawString(sp, this.Font, SystemBrushes.ControlText, (float)(lp_scr + last + (columns[i].Width / 2) - (Utilities.MeasureDisplayStringWidth(g, sp, this.Font) / 2)), (float)(r.Top + borderWid + 3));
							}
						}
					}
				...
				{
					g.Clip = new Region(new Rectangle(r.Left+borderWid, r.Top+borderWid, r.Width, r.Top+headerBuffer));
                    System.Windows.Forms.ControlPaint.DrawButton(g, lp_scr + last, r.Top + borderWid, r.Width - (r.Left + last) - borderWid + hscrollBar.Value, r.Top + headerBuffer, ButtonState.Normal);
				}
			}
		}		
								...
								&& (tp_scr+(rowHeight*i)+2 < r.Top+r.Height-2-hsize))
							{
								...
								else
								{
									...
									else if (columns[j+1].TextAlign == HorizontalAlignment.Right)
									{
                                        ...
                                        g.DrawString(sp, this.Font, (items[i].Selected && isFocused ? SystemBrushes.HighlightText : new SolidBrush(ForeColor)), (float)(lp_scr + last + columns[j + 1].Width - Utilities.MeasureDisplayStringWidth(g, sp, this.Font) - 2), (float)(tp_scr + (rowHeight * i) + 2));
									}
									else
									{
										sp = TruncatedString(items[i].SubItems[j].Text, columns[j+1].Width, 12, g);
                                        g.DrawString(sp, this.Font, (items[i].Selected && isFocused ? SystemBrushes.HighlightText : new SolidBrush(ForeColor)), (float)(lp_scr + last + (columns[j + 1].Width / 2) - (Utilities.MeasureDisplayStringWidth(g, sp, this.Font) / 2)), (float)(tp_scr + (rowHeight * i) + 2));
									}
								}
							}
						}
		...
		protected virtual void DrawBorderStyled(Graphics g, Rectangle r)
		{
            ...
            if (borderstyle != BorderStyle.None)
                g.DrawRectangle(new Pen(SystemBrushes.WindowFrame), r.Left, r.Top, r.Width-1, r.Height-1);
		...
		protected virtual void DrawHeadersStyled(Graphics g, Rectangle r)
		{
			if (headerStyle != ColumnHeaderStyle.None)
			{
				...
				try
				{
					...
					// only render trailing column header if the end of the
					// last column ends before the boundary of the listview 
					if (!(r.Left+last+borderWid-hscrollBar.Value > r.Left+r.Width))
					{
						uxTheme.DrawBackground("HEADER", "HEADERITEMRIGHT", "NORMAL", hdc, lp_scr+last, tp, r.Width-last-borderWid+hscrollBar.Value, headerBuffer,  r.Left, r.Top, r.Width, headerBuffer);
					}
				}
				catch
				{
				...
				for (i=0; i<columns.Count; i++)
				{
					...
					{
						g.DrawImage(columns[i].Image, new Rectangle(lp_scr+last+4, r.Top+borderWid+1, 16, 16));
                        g.DrawString(TruncatedString(columns[i].Text, colwid, 25, g), this.Font, SystemBrushes.ControlText, (float)(r.Left + last + 22 - hscrollBar.Value), (float)(r.Top + borderWid+3));
					}
					else
					{
						...
						if (columns[i].TextAlign == HorizontalAlignment.Left)
						{
                            g.DrawString(TruncatedString(columns[i].Text, colwid, 0, g), this.Font, SystemBrushes.ControlText, (float)(last + 4 - hscrollBar.Value), (float)(r.Top + borderWid + 3));
						}
						else if (columns[i].TextAlign == HorizontalAlignment.Right)
						{
                            g.DrawString(sp, this.Font, SystemBrushes.ControlText, (float)(last + colwid - Utilities.MeasureDisplayStringWidth(g, sp, this.Font) - 4 - hscrollBar.Value), (float)(r.Top + borderWid + 3));
						}
						else
						{
                            g.DrawString(sp, this.Font, SystemBrushes.ControlText, (float)(last + (colwid / 2) - (Utilities.MeasureDisplayStringWidth(g, sp, this.Font) / 2) - hscrollBar.Value), (float)(r.Top + borderWid + 3));
						}
					}
					last += colwid;
				}
