...
namespace DeOps.Interface.Views
{
	public class RichTextBoxEx : RichTextBox
	{
        ...
        public void DetectLinks(string protocol)
        {
            int start = 0;

            bool linksFound = true;

            while (linksFound)
            {
                linksFound = false;

                start = Find(protocol + "://", start, RichTextBoxFinds.None);

                if (start == -1)
                    break;

                int end = Find(new char[] { ' ', '\n' }, start);
        ...
        public void DetectLinksDefault()
        {
            DetectLinks("deops");
            DetectLinks("http");
            DetectLinks("https");
