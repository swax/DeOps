...
namespace DeOps.Interface.TLVex
{
	...
	public class ContainerListViewItem: ICloneable
	{
		...
		private int index;
		private ContainerListViewEx listview;
		...
		public ContainerListViewItem(ContainerListViewEx Listview, int Index)
		{
			index = Index;
			listview = Listview;
		}
...
*/
	...
	{
		// send an public event when a column is resized
	...
	public class ColumnHeaderCollection: CollectionBase
	{
		...
		public virtual ToggleColumnHeader Add(string str, int width, HorizontalAlignment textAlign, ColumnScaleStyle scale)
		{
			ToggleColumnHeader tch = new ToggleColumnHeader();
			tch.Text = str;
			tch.Width = width;
	...
	public class ContainerListViewEx: Control // deriving from UserControl causes selection of rows with embedded controls to mess up
	{
        ...
        protected List<Image> smallImageList, stateImageList;
        ...
        protected ColumnScaleStyle colScaleStyle;
		...
		protected bool doRowTracking = false;
        ...
        [
        Category("Appearance"),
        Description("Disable horizontal scrollbar"),
        DefaultValue(false)
        ]
        public bool DisableHorizontalScroll
        {
            get { return DisableHScroll; }
            set { DisableHScroll = value; }
        }
        ...
        public List<Image> StateImageList
		{
...
*/
		...
		protected override void OnMouseMove(MouseEventArgs e)
		{
			...
			else if (colScaleMode)
			{
                ...
                if (colScaleStyle == ColumnScaleStyle.Spring)
                    colScalePos *= -1;
                ...
                else
                {
                    ...
                    if (columns.Count > scaledCol + 1 && columns[scaledCol + 1].ScaleStyle == ColumnScaleStyle.Spring)
                        columns[scaledCol + 1].Width = colScaleWid - colScalePos;
                }
        ...
        public ContainerListViewItem GetItemAt(Point point)
        {
            if (rowsRect.Contains(point) && items.Count > 0)
            {
                GenerateRowRects();

                for (int i = 0; i < items.Count; i++)
                    if (rowRects[i].Contains(point))
                        return items[i];
            }
        ...
        bool generating = false; // prevents setting width again from causing overflow

		public void GenerateColumnRects()
		{
            if (generating)
                return;
			...
			for (int i=0; i<columns.Count; i++)
			{
                if (columns[i].ScaleStyle == ColumnScaleStyle.Spring)
                    columns[i].Width = springWid;

				colwid = columns[i].Width;
...
/*
				...
				for (i=0; i<items.Count; i++)
				{
						...
						&& (tp_scr+(rowHeight*i)+2 < r.Top+r.Height-2-hsize))
					{
						...
						{	
							...
							if (smallImageList != null && (items[i].ImageIndex >= 0 && items[i].ImageIndex < smallImageList.Count))
							{
                                g.DrawImage(smallImageList[items[i].ImageIndex], lp_scr+4, tp_scr+(rowHeight*i)+1, 16, 16);

                                if(items[i].Overlays != null)
                                    foreach (int overlay in items[i].Overlays)
                                        if(overlay >= 0 && overlay < OverlayImages.Count)
                                            g.DrawImage(OverlayImages[overlay], lp_scr + 4, tp_scr + (rowHeight * i) + 1, 16, 16);

                                g.DrawString(TruncatedString(items[i].Text, iColWidth /* BMS 2003-05-24 */ , 18, g), font, (items[i].Selected && isFocused ? SystemBrushes.HighlightText : new SolidBrush(items[i].ForeColor)), (float)(lp_scr + 22), (float)(tp_scr + (rowHeight * i) + 2));
							}
							else
							{
                                g.DrawString(TruncatedString(items[i].Text, iColWidth /* BMS 2003-05-24 */ , 0, g), font, (items[i].Selected && isFocused ? SystemBrushes.HighlightText : new SolidBrush(items[i].ForeColor)), (float)(lp_scr + 4), (float)(tp_scr + (rowHeight * i) + 2));

                                //g.DrawString("Hey There", new Font("Tahoma", 14), (items[i].Selected && isFocused ? SystemBrushes.HighlightText : new SolidBrush(items[i].ForeColor)), (float)(lp_scr + 4), (float)(tp_scr + (rowHeight * i) + 2));
                            }
						}
					}
		...
		protected virtual void DrawBorderStyled(Graphics g, Rectangle r)
		{
            ...
            if (borderstyle == BorderStyle.FixedSingle || borderstyle == BorderStyle.Fixed3D)
            {
                ...
                // causing white line around window g.DrawRectangle(new Pen(BackColor), r.Left+1, r.Top+1, r.Width-3, r.Height-3);
                g.Clip = oldreg;
            }
			...
			if (headerStyle != ColumnHeaderStyle.None)
			{
				...
				for (i=0; i<columns.Count; i++)
				{
					...
					else
					{
						...
						{
							g.DrawString(sp, this.Font, SystemBrushes.ControlText, (float)(last+colwid-StringTools.MeasureDisplayStringWidth(g, sp, this.Font)-16-hscrollBar.Value), (float)(r.Top+5));
						}
						else
						{
