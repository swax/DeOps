...
namespace DeOps.Implementation.Dht
{
	...
	{
        public Dictionary<ulong, List<DhtContact>> ContactMap = new Dictionary<ulong, List<DhtContact>>();
		...
		public DhtBucket(DhtRouting routing, int depth, bool lastBucket)
		{
			Depth      = depth;
			Routing    = routing;
			Last = lastBucket;
		...
		public UInt64 GetRandomBucketID()
		{
	...
	public class DhtContact : DhtAddress
	{
        ...
        public DateTime  NextTryIP;
        public DateTime  NextTryProxy; // required because attempts more spaced out
        public ushort    Ident;

        public DhtContact() { }

        public DhtContact(UInt64 user, ushort client, IPAddress address, ushort tcpPort, ushort udpPort)
		{
			UserID     = user;
			ClientID  = client;
			IP   = address;
        ...
        public byte[] Encode(G2Protocol protocol, byte name)
        {
        ...
        public void Alive(DateTime latest)
        {
            if (latest > LastSeen)
            {
                LastSeen = latest;
